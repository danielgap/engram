...
- **feat(doctor):** add persisted acknowledgements so an accepted doctor finding stops re-reporting. `engram doctor acknowledge --check <code> [--project P] [--note N]` fingerprints every finding the scoped run reports (sha256 over the finding's evidence) and upserts one local `doctor_acknowledgements` row per finding; `--revoke` removes one row by fingerprint or all rows for the check. Acknowledged findings are filtered out of doctor reports — active findings alone drive severity, an all-acknowledged check reports `ok` with `acknowledged_count`, and a full unscoped run prunes acknowledgements whose evidence no longer exists (`acknowledgements_pruned`), while single-check runs never prune because a scoped run does not prove absence. Acknowledging is an explicit allowlist (`orphaned_observation_session`, `ambiguous_active_runtime_sessions`) so repairable checks stay honest: their exit is repair, not a persisted silence.
