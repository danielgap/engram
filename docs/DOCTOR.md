...
- `stale_open_sessions` — warns when open sessions have had no recorded activity (newest observation, falling back to `started_at`) for more than 30 days. One aggregate finding carries the stale count, per-project counts, and the oldest/newest stale activity timestamps, and it sets `requires_confirmation=true`. Doctor is strictly read-only: it never ends sessions. Review with `engram session end --by-age 720h --project <name>` (dry-run by default), then add `--apply` when appropriate.
